...
<screen id="Q2_PRO">
<message>
claude.ai の有料プラン（Pro プラン: 月額約3,000円）への登録が必要です。以下の手順で登録してください。

1. ブラウザで https://claude.ai にアクセスする
2. アカウントをお持ちでない場合は「Sign up」からアカウントを作成する
3. ログイン後、左下のご自身の名前をクリック → 「Settings」を選択
4. 「Subscription」または「サブスクリプション」をクリック
5. 「Pro」プランの「Subscribe」または「登録」ボタンをクリック
6. 支払い情報（クレジットカード等）を入力して登録を完了する
